...
export default function Features() {
  ...
  return (
    <div className={styled.featuresContainer}>
      {featuresData.map((feature, index) => (
        <div key={index} className={styled.feature}>
          <img src={feature.icon} alt={feature.title} className={styled.feature__icon} />
          <h3 className={styled.feature__title}>{feature.title}</h3>
          <p>{feature.text}</p>
        </div>
      ))}
    </div>
